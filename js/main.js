...
(function () {
  ...
  const KEY = 'island-expedition-v3';
  const Game = {
    ...
    readSave() {
      try {
        ...
        return s?.version === 3 && s.day >= 1 && s.day <= 30 && s.deck?.hand && s.forecasts?.length === 30 ? s : null;
      } catch (_) { return null; }
    },
    render() {
      this.save();
      if (this.state.status !== 'playing') U.renderResults(this.state);
      else U.render(this.state, this);
    },
    startNew(seed) {
      if (this.busy) return;
      U.closeModal(); this.state = E.createInitialState(seed);
      U.showScreen('game-screen'); this.render();
    },
    resume() {
    ...
    async selectCard(uid) {
      ...
      if (!c) return;
      if (this.state.energy < c.cost) { U.toast('行动力不足；可留到明天，或结束今天恢复行动力。'); return; }
      if (c.hurt && this.state.hp <= c.hurt) { U.toast('生命不足，无法支付动员代价。'); return; }
      this.busy = true;
      try {
        const effect = E.play(this.state, uid);
        if (effect) {
    ...
    endRound() {
      ...
      E.endDay(this.state); this.render();
    ...
    chooseReward(type, id) {
      ...
      delete this.state.rewardChoices; U.closeModal(); this.render();
    },
    retain(uid) {
    ...
    action(name) {
      ...
      if (name === 'guide') { U.openGuide(); return; }
      ...
      if (name === 'swap') E.swap(this.state);
  ...
  document.addEventListener('pointermove', event => {
    ...
    if (!drag.active && up > 7 && up > Math.abs(event.clientX - drag.startX) * .65) {
      ...
      document.body.appendChild(drag.card); drag.card.classList.add('dragging-card');
